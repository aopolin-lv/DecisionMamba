            ...
            if cache_params is not None and cache_params.seqlen_offset > 0:
                ...
                if getattr(self, "__DEBUG__", False):
                    setattr(self, "__data__", dict(
                        A_logs=self.A_log, Bs=B.transpose(1, 2), Cs=C.transpose(1, 2), Ds=self.D.float(),
                        us=hidden_states, dts=discrete_time_step, delta_bias=time_proj_bias,
                        ys=scan_outputs
                    ))
...
class MambaBlock(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.mixer_group = MambaMixer(config, layer_idx=layer_idx)

        self.conv1d = nn.Conv1d(in_channels=768, out_channels=768, kernel_size=3, stride=3)
        self.ln_norm = nn.LayerNorm(config.hidden_size)

    def forward(self, hidden_states, cache_params=None, cache_params_group=None):
        batch_size, step, hidden_dim = hidden_states.shape
        window_size = 3
        context_length = step // window_size
        ...
        hidden_states_group = torch.cat((hidden_states[:,:-1,:], torch.zeros(batch_size, 1, hidden_dim).to(hidden_states.device)), dim=1)
        hidden_states = self.mixer(hidden_states, cache_params=cache_params)

        # hidden_states_group = hidden_states_group.reshape(batch_size, context_length, 3, -1)
        # hidden_states_group = self.gather_group_lr(hidden_states_group.permute(0, 1, 3, 2))
        # hidden_states_group = hidden_states_group.permute(0, 1, 3, 2).reshape(batch_size, context_length, hidden_dim)
        hidden_states_group = hidden_states_group.permute(0, 2, 1)
        hidden_states_group = self.conv1d(hidden_states_group).permute(0, 2, 1)
        hidden_states_group = self.mixer_group(hidden_states_group, cache_params=cache_params_group)
        hidden_states_group = hidden_states_group.repeat(1, 3, 1)
        hidden_states_group = self.ln_norm(hidden_states_group)

        hidden_states = hidden_states + hidden_states_group
        ...
        **kwargs,  # `attention_mask` is passed by the tokenizer and we don't want it
        ...
        for mixer_block in self.layers:
            if self.gradient_checkpointing and self.training:
                ...
                hidden_states = mixer_block(hidden_states, cache_params=cache_params, cache_params_group=cache_params_group)
        ...
        if use_cache:
            ...
            cache_params_group.seqlen_offset += inputs_embeds.shape[1]
